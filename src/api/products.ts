...
const { REACT_APP_COLLECTION_PRODUCTS } = process.env;
...
const productsApi = {
  listByCategory: (subCategory: string): Promise<ListResponse<ProductModel>> => {
    return getAppWrite()
      .database
      .listDocuments(
        REACT_APP_COLLECTION_PRODUCTS,
        [Query.equal('subCategory', subCategory)],
      );
  },
  get: (product: string): Promise<ProductModel> => {
    return getAppWrite().database.getDocument(REACT_APP_COLLECTION_PRODUCTS, product);
  },
