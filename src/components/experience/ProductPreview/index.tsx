import { FC, useMemo } from 'react';
import { FlexBox, formatCurrency, Paragraph, Text, Title } from 'activate-components';
import { ProductModel } from 'models/product';
import productsApi from 'api/products';
import { ImageHolder } from './styled';

const ProductPreview: FC<ProductModel> = (props) => {
  const { name, price, description, thumbnail } = props;
  const imgUrl = useMemo(() => productsApi.fetchThumbnail(thumbnail), []);

  return (
    <FlexBox direction="column" align="stretch" mT mB>
      <FlexBox align="stretch">
        <ImageHolder>
          <img src={imgUrl.href} width={80} height={80} alt={name} />
        </ImageHolder>
        <FlexBox direction="column" align="stretch" padding="0 0 8px 8px">
          <Title level={3} margin="0 0 8px 0">{name}</Title>
          <Text margin="0 0 8px 0">{formatCurrency(price)}</Text>
        </FlexBox>
      </FlexBox>
      <Paragraph margin="12px 0 0 0">{description}</Paragraph>
    </FlexBox>
  )
};

export default ProductPreview;
